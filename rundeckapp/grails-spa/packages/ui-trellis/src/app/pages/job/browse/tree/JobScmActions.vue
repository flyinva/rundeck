<template>
    <template v-if="exportSynchState && exportAuthz">
        ...
        <li v-for="action in exportActions">
            <a
                ...
                :href="scmActionHref(action.id, 'export')"
            >
                {{ action.title }}
            </a>
        </li>
    ...
    <template v-if="importSynchState && importAuthz">
        ...
        <li v-for="action in importActions">
            <a
                :title="action.description"
                :href="scmActionHref(action.id, 'import')"
            >
                {{ action.title }}
            </a>
        </li>
...
export default defineComponent({
    ...
    methods: {
        ...
        scmActionHref(id: string, integration: string): string {
            return `${this.scmHrefBase(
                integration
            )}/performAction?actionId=${id}`;
        },
    ...
    computed: {
        exportAuthz():boolean{
          return this.jobPageStore.projAuthz['export']||this.jobPageStore.projAuthz['scm_export']
        },
        importAuthz():boolean{
          return this.jobPageStore.projAuthz['import']||this.jobPageStore.projAuthz['scm_import']
        },
